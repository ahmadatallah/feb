...
Full workflow examples in [`examples/`](./examples): [`build-android.yml`](./examples/build-android.yml) (on push), [`build-ios.yml`](./examples/build-ios.yml), and [`build-on-demand.yml`](./examples/build-on-demand.yml) — `workflow_dispatch` with platform + profile pickers, both jobs, the low-memory and free-disk-space steps a hosted runner needs. iOS builds need `runs-on: macos-latest` (Xcode preinstalled on GitHub-hosted macOS runners).
...
`org.gradle.workers.max` is worth pinning at 2 on the small runner — it caps the parallel phases (C++ compile, resource merging) — but it does not reach the task that dies: modern AGP merges dex *in-process* inside the daemon heap, so the worker cap cannot touch it. Only the heap cap can.
...
const jvmArgs = [
  ...
  // If the daemon still dies, leave a heap dump instead of a silent kill
  '-XX:+HeapDumpOnOutOfMemoryError',
  '-XX:+UseG1GC',
  // JDK 21+ needs these for CMake / the FFM API; harmless on 17
  '--add-opens=java.base/java.lang=ALL-UNNAMED',
  '--enable-native-access=ALL-UNNAMED',
].join(' ')

// Upsert a gradle.properties entry — android/ is gitignored, so this file is generated
const setProperty = (properties, key, value) => {
  const i = properties.findIndex((item) => item.type === 'property' && item.key === key)
  if (i !== -1) {
...
module.exports = (config) =>
  withGradleProperties(config, (config) => {
    ...
    console.log(
      `[withGradleJvmArgs] ${isLowMemoryRunner ? 'LOW-MEMORY' : 'default'} profile ` +
        `(totalmem ${(os.totalmem() / GIB).toFixed(1)} GiB): ${jvmArgs}`
    )
    config.modResults = setProperty(
      setProperty(config.modResults, 'org.gradle.jvmargs', jvmArgs),
...
Job-level `env` reaches the composite action's shell, `eas build --local` inherits it, and prebuild's child processes inherit it from there. The plugin's log line lands in the `[PREBUILD]` phase of the build output — `LOW-MEMORY profile (totalmem 7.8 GiB)` is what a private-repo runner prints, and it is the first thing to check when a run dies.
...
With both in place — the capped daemon and the two ARM ABIs — a production AAB of [Hashcards](https://hashcards.app) (Skia, Reanimated, Rive, OneSignal — ~1,600 Gradle tasks) builds on a **private-repo** `ubuntu-latest` in about 52 minutes end to end (`BUILD SUCCESSFUL in 45m 34s`); the iOS production `.ipa` takes about 27 minutes on `macos-latest` alongside it. The workflow that does it, platform and profile picked from the Actions UI, is [`examples/build-on-demand.yml`](./examples/build-on-demand.yml).
...
**[Hashcards](https://hashcards.app)** — flashcards with free spaced-repetition scheduling (FSRS). Ships its iOS and Android builds with this action — [`examples/build-on-demand.yml`](./examples/build-on-demand.yml) is its workflow, and the `withGradleJvmArgs` plugin above is the one it runs.
